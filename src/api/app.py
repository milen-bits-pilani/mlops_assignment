...
PREDICTION_COUNT = Counter(
    "predictions_total", "Total number of predictions made", ["prediction_class"]
)
...
async def lifespan(app: FastAPI):
    ...
    if model is not None and preprocessor is not None:
        ...
        logger.warning(
            "API started but models not loaded. /predict endpoint will not work until models are copied to pod."
        )
    yield
    # Shutdown (if needed)
...
def load_model():
    ...
    try:
        ...
        if os.path.exists(model_path):
            ...
            logger.warning(
                f"Model file not found at {model_path}. API will start but /predict will fail."
            )
            logger.warning(
                "Copy models to pod using: kubectl cp models/best_model.pkl <pod>:/app/models/"
            )
            model = None
        ...
        if os.path.exists(preprocessor_path):
            ...
            logger.warning(
                "Copy preprocessor to pod using: kubectl cp models/preprocessor.pkl <pod>:/app/models/"
            )
            preprocessor = None
...
async def reload_models():
    ...
    if model is not None and preprocessor is not None:
        ...
        raise HTTPException(
            status_code=503, detail="Models not found. Please copy models to pod first."
        )

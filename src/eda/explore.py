...
def perform_eda(data_path: str, output_dir: str = "data/eda"):
    ...
    print(class_dist)
    print(
        f"   Class balance: {class_dist[0] / len(df):.2%} (No Disease) vs {class_dist[1] / len(df):.2%} (Disease)"
    )
    ...
    plt.suptitle(
        "Feature Distributions (Histograms)", fontsize=16, fontweight="bold", y=1.02
    )
    plt.tight_layout()
    ...
    for idx, feature in enumerate(key_features):
        if feature in df.columns:
            df[df["target"] == 0][feature].hist(
                ax=axes[idx], alpha=0.5, label="No Disease", bins=20
            )
            df[df["target"] == 1][feature].hist(
                ax=axes[idx], alpha=0.5, label="Disease", bins=20
            )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{output_dir}/feature_distributions_by_class.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.suptitle(
        "Box Plots: Key Features by Target Class", fontsize=16, fontweight="bold"
    )
    plt.tight_layout()
